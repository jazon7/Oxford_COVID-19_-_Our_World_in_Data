...
library(pacman)
p_load(tidyverse,readxl,lubridate,data.table,scales,viridis,ggthemes,shiny,shinyWidgets,ggrepel)
...
#import owid data 
df <- fread("owid-covid-data.csv",stringsAsFactors = TRUE) %>% as_tibble()
df <- df %>% select(-c(tests_units,stringency_index))
...
cv_country_list <- df %>%  filter(date == "2020-12-30") %>%  select(location,iso_code)
...
df <- merge(df, ox_df, by = c('iso_code','location','date'))
...
cor_variables <- c("location","iso_code","date","continent","total_cases_per_million",
                   ...
                   "life_expectancy","human_development_index","latitude","longitude",
                   "total_restrictions","total_restrictions_cubed","first_death_date",
                   "date_first_death_days","obesity_prevalence")
...
country_scatter_list <- create_metric_list(df %>% select(cor_variables))
             ...
             sidebarPanel = sidebarPanel(
               ...
               conditionalPanel(condition = "input.tabselected==2",
                                ...
                                selectizeInput('y_metric', 'y Metric',
                                               ...
                                               selected = country_scatter_list[5]),
                                
                                
                                selectizeInput('x_metric', 'x Metric',
                                               choices = country_scatter_list,
                                               
                                               selected = country_scatter_list[10]),
                                
                                selectizeInput('size_metric', 'Size Metric',
                                               choices = country_scatter_list,
                                               
                                               selected = country_scatter_list[25]),
...
server <- function(input, output) {
  ...
  filter_continent_date <- reactive({
    df <- df_filtered(df,input$date_select) %>% select(cor_variables)
    df <- df %>%
  ...
  output$country_scatter <- renderPlot({
      ...
      ylab(names(df)[which(unlist(df, use.names = F) == input$y_metric)]) +
      xlab(names(df)[which(unlist(df, use.names = F) == input$x_metric)]) +
      labs(color = "Continent", size = names(df)[which(unlist(df, use.names = F) == input$size_metric)]) +
      ggtitle(paste0(names(df)[which(unlist(df, use.names = F) == input$y_metric)], " vs ",
                     names(df)[which(unlist(df, use.names = F) == input$x_metric)], ' as of ', input$date_select)) +
